...
# Table of Contents
#1.  Simulate Counts - Line 14
#2.  Simulate counts with individual variation in stage duration - Line 286
#3.  Analysis of simulated counts - Line 677
#4.  Analysis of field data - Line 845
...
#also used for a buffer at the end of ADD, which has no biological meaning
pupamax = 16
...
#daily survival probabilities for each stage
...
#i.e., no days with no eggs laid between days with eggs laid and the first day must have eggs laid
CH = sum(B != 0)
...
#large loop to create many simulated datasets - closing bracket at ~ L671
for (m in 1:100){
...
#n for analysis is study days plus pupamax - this is a critical difference between simulation and analysis code
#MT thus must have temps for the 16 days previous to the beginning of the study

pupamax = 16
n = 50+pupamax #note that n for analysis is the days in the study plus the buffer pupamax  
MT = rep(21,n)
...
#check R-hat
psrf = R[[1]]$psrf$psrf[89,1]
for (i in 2:100){
  psrf[i] = R[[i]]$psrf$psrf[89,1]
}
psrf
gud = psrf < 1.10
sum(gud)
